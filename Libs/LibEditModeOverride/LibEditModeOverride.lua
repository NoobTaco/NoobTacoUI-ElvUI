...
function lib:SetFrameSetting(frame, setting, value)
  ...
  if restrictions then
    ...
    -- Validation removed to accommodate newer Blizzard UI components.
...
function lib:ApplyChanges()
  ...
  -- Note: We no longer Show/Hide EditModeManagerFrame here.
  -- In Retail 11.0+, these calls are highly prone to Taint (secret value errors).
  -- The changes are saved via C_EditMode.SaveLayouts, and will be visible
  -- after a /reload or manually opening Edit Mode.
end
